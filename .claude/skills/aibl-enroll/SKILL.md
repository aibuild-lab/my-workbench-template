...
7. **Verify and start.** Inspect the commit and verify personal files, settings, and root instructions were unchanged. Report the installed student-branch commit and any deliberately retained customizations. Do not claim installation means a worker ran. In Claude Code, first offer the agent menu exactly as `aibl-update` step 6.5 describes: run `node .claude/hooks/update-check.mjs --agent-menu` yourself, explain what it lists in plain words (do not hand the student the command), and only on their yes run `--agent-menu-apply` yourself, then have them quit the app fully and reopen it. Ask the student to start a new session in the same folder, then run `aibl-workforce` (slash skill in Claude, dollar-sign skill in Codex). Check actual named-agent discovery and a real response using the installed program's supported instructions. Missing discovery stops for diagnosis; never write agents outside the workbench unasked. Offer `aibl-checkpoint` for the student's private origin, separately from enrollment.
