...
class AgentMenu(unittest.TestCase):
    def setUp(self):
        ...
        self.git("remote", "set-url", "agent-workforce", "https://github.com/aibuild-lab/agent-workforce.git")
        ...
        receipt = {"schema": 1, "files": {name: {"workbench": str(self.wb.resolve()),
                   "sha256": hashlib.sha256((self.menu / name).read_bytes()).hexdigest()}
                   for name in ("aibl-chief-of-staff.md", "aibl-retired-seat.md")}}
        (self.home / ".claude/aibl-agent-menu.json").write_text(json.dumps(receipt))
    ...
    def test_never_writes_through_a_link(self):
        ...
        self.assertEqual(dest.read_text(), "another workbench's chief\n")
    ...
    def test_never_ours_alone_is_in_step(self):
        ...
        self.assertTrue(set(NOT_OURS) <= set(report["not_ours"]))
    ...
    def test_same_name_custom_and_other_workbench_copies_survive(self):
        chief = self.menu / "aibl-chief-of-staff.md"
        chief.write_text("custom chief\n")
        own = self.wb / ".claude" / "agents" / "aibl-custom.md"
        own.write_text("student created\n")
        report = json.loads(self.run_hook("--agent-menu-apply"))
        self.assertIn(chief.name, report["conflicts"])
        self.assertEqual(chief.read_text(), "custom chief\n")
        self.assertFalse((self.menu / own.name).exists())

    def test_untrusted_remote_cannot_claim_files(self):
        self.git("remote", "set-url", "agent-workforce", "https://example.invalid/untrusted")
        report = json.loads(self.run_hook("--agent-menu-apply"))
        self.assertEqual(report["applied"]["copied"], [])
        self.assertEqual(report["applied"]["removed"], [])

    def test_identical_unowned_legacy_copy_is_not_retired(self):
        (self.home / ".claude/aibl-agent-menu.json").unlink()
        original = (self.menu / "aibl-retired-seat.md").read_bytes()
        report = json.loads(self.run_hook("--agent-menu-apply"))
        self.assertEqual(report["applied"]["removed"], [])
        self.assertEqual((self.menu / "aibl-retired-seat.md").read_bytes(), original)

    def test_different_workbench_receipt_is_preserved(self):
        receipt_path = self.home / ".claude/aibl-agent-menu.json"
        receipt = json.loads(receipt_path.read_text())
        for row in receipt["files"].values(): row["workbench"] = str(Path(self.temp.name) / "another")
        receipt_path.write_text(json.dumps(receipt))
        report = json.loads(self.run_hook("--agent-menu-apply"))
        self.assertEqual(report["applied"]["removed"], [])
        self.assertEqual((self.menu / "aibl-chief-of-staff.md").read_text(), "chief v1\n")

    def test_no_agents_means_no_line(self):
        for f in (self.wb / ".claude" / "agents").glob("aibl-*.md"):
            f.unlink()
        self.assertEqual(json.loads(self.run_hook("--agent-menu"))["status"], "out_of_step")
